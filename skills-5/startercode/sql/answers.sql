...
SELECT orders.id from orders
 ...
 WHERE customers.fname = 'Jane' and customers.lname = 'Doe';
...
SELECT SUM(num_cupcakes) AS sum FROM orders 
  WHERE processed = 'f';
...
SELECT cupcakes.name, SUM(orders.num_cupcakes) FROM cupcakes
 LEFT JOIN orders
...
SELECT DISTINCT fname, lname, email FROM customers
 LEFT JOIN orders
 ON customers.id = orders.customer_id
 WHERE orders.cupcake_id = (SELECT id from cupcakes WHERE name = 'funfetti') AND orders.processed ='t';